...
class _WordleGamePageState extends State<WordleGamePage> {
    ...
    while (containsForbiddenLetters) {
      if (isTurkish == true) {
        do {
          ...
          print("Random Kelime (Türkçe): ${randomWordData?['meaning']}"); // Rastgele Türkçe kelimeyi kontrol et
        } while (randomWordData != null &&
            (randomWordData['meaning'].length < minLength ||
                randomWordData['meaning'].length > maxLength ||
                randomWordData['meaning'][0] == randomWordData['meaning'][0].toUpperCase() || // Büyük harfle başlayan kelimeleri kabul etme
                randomWordData['meaning'][0].toLowerCase() == 'x')); // 'x' harfiyle başlayan kelimeleri kabul etme
        ...
        do {
          randomWordData = await dataProvider.getRandomWord();
          print("Random Kelime (İngilizce): ${randomWordData?['word']}"); // Rastgele İngilizce kelimeyi kontrol et
        } while (randomWordData != null &&
            (randomWordData['word'].length < minLength ||
                randomWordData['word'].length > maxLength ||
                randomWordData['word'][0] == randomWordData['word'][0].toUpperCase() || // Büyük harfle başlayan kelimeleri kabul etme
                randomWordData['word'][0].toLowerCase() == 'x')); // 'x' harfiyle başlayan kelimeleri kabul etme

        if (randomWordData != null) {
          wordToGuess = randomWordData['word'];
          meaning = randomWordData['meaning']; // Kelimenin İngilizce anlamını al
      ...
      onChanged: (String? value) {
        ...
        setState(() {
          ...
          if (dropdownValue2 == 'Turkish') {
            isTurkish = true;
          } else if(dropdownValue2 == 'English'){
            isTurkish = false;
          }
        });
        _initializeGame(); // Dil değiştiğinde oyunu yeniden başlat
  ...
  void _showLengthWarning() {
    final snackBar = SnackBar(
      content: Text("Tahmin uzunluğu belirlenen sınırların dışında.",style: widgets.text_style()),
  ...
  void _showCongratulationsDialog() {
    showDialog(
      ...
      builder: (BuildContext context) {
        String message =  meaning; // Kelimenin anlamı veya kelimesi
        return AlertDialog(
          title: Text("Tebrikler! Kazandınız!", style: widgets.text_style(),),
          content: Text("Doğru kelimeyi buldunuz: $wordToGuess \n"
              "Karşılığı: $message", style: widgets.text_style()),
          actions: <Widget>[
            TextButton(
              child: Text("Yeni Oyun",style: widgets.text_style()),
              onPressed: () {
                setState(() {
                  currentAttempt = 0;
                  _initializeGame();
  ...
  void _showLoseDialog() {
    showDialog(
      ...
      builder: (BuildContext context) {
        String message = meaning; // Kelimenin anlamı veya kelimesi
        return AlertDialog(
          title: Text("Üzgünüz! Kaybettiniz.", style: widgets.text_style(),),
          content: Text("Doğru kelimeyi bulamadınız. Kelime:$wordToGuess \nKarşılığı: $message", style: widgets.text_style()),
          actions: <Widget>[
            TextButton(
              child: Text("Yeni Oyun", style: widgets.text_style(),),
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children:[
      Scaffold(
        appBar: widgets.buildAppBar("WORDLING - KELİME BİLMECE"),
              ...
              Stack(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    icon: Icon(Icons.check_circle, color: Colors.green, size: 37,),
                  ),
                ],
              ),
            SizedBox(height: 20,),
